...
def pltTra(positions, srcAddrList):
    ...
    colors = cmap(np.linspace(0, 1, len(srcAddrList)))
    ...
    for idx, srcAddr in enumerate(srcAddrList):
        ...
        scatter = ax.scatter(M_interp[:, 0], M_interp[:, 1], M_interp[:, 2], c=[colors[idx]]*len(M_interp), s=6, label=f'Trajectory {srcAddr}', alpha=0.6)
        
        # 起始点
        ax.plot([M[0, 0]], [M[0, 1]], [M[0, 2]], 'o', markersize=8, markeredgecolor='k', markerfacecolor=colors[idx])
        
        # 终止点
        ax.plot([M[-1, 0]], [M[-1, 1]], [M[-1, 2]], 'o', markersize=8, markeredgecolor='k', markerfacecolor=colors[idx])
