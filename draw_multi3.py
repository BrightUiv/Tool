...
def pltTra(positions, srcAddrList):
    num_plots = len(srcAddrList)
    cols = 3  # 每行的子图数量
    rows = (num_plots + cols - 1) // cols  # 计算行数
    fig = plt.figure(figsize=(5 * cols, 5 * rows))  # 根据行列数调整图像大小
    
    # 生成颜色映射
    cmap = plt.get_cmap('jet')

    for idx, srcAddr in enumerate(srcAddrList):
        ax = fig.add_subplot(rows, cols, idx + 1, projection='3d')  # 添加一个三维子图，每个子图对应一个 srcAddr
        ...
        # 设置标题
        ax.set_title(f'Trajectory {srcAddr}')
        # 添加图例
        ax.legend()
        
        # 添加 colorbar
        norm = plt.Normalize(t_interp.min(), t_interp.max())
